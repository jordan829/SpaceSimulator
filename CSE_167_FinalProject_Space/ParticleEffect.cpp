//Implemented by following tutorial at http://www.3dgep.com/simulating-particle-effects-using-opengl/
//Much of this code was reused from the source, with a few slight modifications to both readability and functionality
...
void ParticleEffect::BuildVertexBuffer()
{
	...
	cameraRotation = glm::quat(glm::radians(Globals::camera.GetRotation()));
	...
	for (unsigned int i = 0; i < particles.size(); ++i)
	{
		...
		{
			// Bottom-left
			v0.pos = particle.position + cameraRotation * ((-X - Y) * particle.size);
			v0.tex = glm::vec2(0, 1);
			v0.diffuse = particle.color;

			// Bottom-right
			v1.pos = particle.position + cameraRotation * ((X - Y) * particle.size);
			v1.tex = glm::vec2(1, 1);
			v1.diffuse = particle.color;

			// Top-right
			v2.pos = particle.position + cameraRotation * ((X + Y) * particle.size);
			v2.tex = glm::vec2(1, 0);
			v2.diffuse = particle.color;

			// Top-left
			v3.pos = particle.position + cameraRotation * ((-X + Y) * particle.size);
			v3.tex = glm::vec2(0, 0);
...
void ParticleEffect::Update(float changeInTime)
{
	for (unsigned int i = 0; i < particles.size(); ++i)
	{
		Particle& particle = particles[i];

		particle.age += changeInTime;
		if (particle.age > particle.lifeTime)
			EmitParticle(particle);

		float lifeRatio = glm::saturate(particle.age / particle.lifeTime);
		particle.velocity = particle.velocity + (force * changeInTime);
		particle.position = particle.position + (particle.velocity * changeInTime);
...
void ParticleEffect::Render()
{
	...
	{
		glLoadMatrixf(toWorld.ptr());	//Follows camera movement

		glScalef(scale, scale, scale);	//Scale down particle size
	}
