...
OBJObject::OBJObject(/*std::string filename*/) : Drawable()
{
	...
	//// Bring object in front of camera
	//Matrix4 zTrans;
	//zTrans.makeTranslate(0, 0, -20);
	//toWorld = zTrans* toWorld;

	//Bring down for slight top view
	//rotate.makeRotateX(-PI / 16);
	//toWorld = rotate * toWorld;

	////Rotate up for better visibility
	//rotate.makeRotateZ(-PI / 16);
	//toWorld = toWorld * rotate;

	/*Matrix4 scale;
	scale = scale.makeScale(0.1);
